...
import './dmrtd/dmrtd.dart';
import './dmrtd/internal.dart';
...
String formatDG13(final DG13data dg13data) {
  return "dataEncoded:\n"+ " ${dg13data.dg13dataAfter}" ;
}
...
class NFCReaderScreen extends StatelessWidget {
  const NFCReaderScreen({super.key, required this.infoInput});
  final InfoInput infoInput;
...
class MrtdHomePage extends StatefulWidget {
  const MrtdHomePage({super.key, required this.infoInput});
  final InfoInput infoInput;
...
class _MrtdHomePageState extends State<MrtdHomePage> {
  var _alertMessage = "";
  final _log = Logger("id_scanner.app");
  ...
  @override
  void initState() {
    docNumber = widget.infoInput.documentNumber;

    dob = DateFormat("MM/dd/yyyy").format(widget.infoInput.birthdate);

    doe = DateFormat("MM/dd/yyyy").format(widget.infoInput.expirydate);
  ...
  void _readMRTD() async {
    try {
      ...
      try {
        ...
        final bacKeySeed = DBAKey(docNumber!, _getDOBDate()!, _getDOEDate()!);
  ...
  List<Widget> _mrtdDataWidgets() {
    ...
    if (_mrtdData!.dg13 != null) {
      list.add(_makeMrtdDataWidget(
          header: 'EF.DG13',
          collapsedText: '',
          dataText: formatDG13(_mrtdData!.dg13!.dg13data)));
          // 
    }
  ...
  Scaffold _buildPage(BuildContext context) => Scaffold(
        ...
        appBar: AppBar(
          ...
          leading: IconButton(
            ...
            onPressed: () {
              // Navigate to a specific page instead of popping the current screen
